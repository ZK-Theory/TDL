...
- **T1.20 BHPS parental-NS-SEC hardcoded-NaN concern — AUDITED, RESOLVED (2026-07-11, Manager 13): no v2 exposure.** `covariate_extractor.py` hardcoded BHPS parental NS-SEC to NaN until the T1.28 `[DATA]` fix (`289bed2`/`909811b`, 2026-06-28). The read-only audit found the escape-regression chain (T1.18–T1.35) sources parental class from an *independent* `nssec_proxy` variable read from `xwavedat.tab` (`pasoc90_cc`/`masoc90_cc`) in the R scripts — never `covariate_extractor.py` — so no §4.5 claim is affected; every parental-NS-SEC number traces to `nssec_proxy` fields. Only pre-fix consumers of the buggy field are v1-era files predating Stage 1; the one v2-cited number from them (P01-B "86.1% Inactive-Low 60+") is age-only. T1.28's BHPS-nssec FDR (bound by T2.7 §6.1) ran post-fix and is correct. No rerun. [DECISION] filed (Computational-Log 2026-07-11). Thread closed.
