...
**ε\* knee detection.** Algorithm formalised (pseudocode in P01-B §3.4.2) and extracted as `detect_eps_star_knee()` in `run_zigzag_sensitivity.py` (T0.6). Spanning Betti comparison reported in three forms: single-ε ratio (current), AUC ratio over the full filtration, and W₂(D₀(X_t^new), D₀(X_t^*)) at matched sample sizes. ε* robustness across {0.54, 0.65, 0.70, 0.80}. **Empirical finding (T0.6):** `knee_analysis.json` gives median ε\*=0.54 (32 years, 1991–2022); 4 degenerate years (2003, 2005, 2011, 2019). The value 0.70 cited in the original H1 run is a pre-sensitivity-analysis hard-coded constant. **DECISION PENDING (User):** whether to cite ε\*=0.54 (data-driven) or 0.70 (original run) as the point estimate in P01-A §4.3.2 and P01-B §3.4.2 — the robustness set {0.54, 0.65, 0.70, 0.80} is unaffected by this decision. Prose for §4.3.2 blocked until decision locked. Reference: P01-B response plan §9 (M3).
