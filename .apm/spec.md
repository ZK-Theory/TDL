...
**`jbstat` harmonisation and BHPS/USoc state-space comparability.** The harmonised UKDA-6614 dataset is the working assumption for cross-survey state-space comparability, but **must be verified** before the corresponding R2 work proceeds: per-wave `jbstat` mappings examined, ILO-unemployment treatment in early BHPS waves checked, zero-hours / gig coding examined for the EL state. Reference: P01-A response plan §S4.

**BHPS / USoc income concept reconciliation.** The harmonised dataset is the working assumption for income-concept comparability across BHPS `fihhmn` and USoc `fihhmnnet3_dv`, but **must be verified** before §6.2 reframing. Cross-era calibration check on the spanning sub-sample (last BHPS income band vs first USoc income band) reported. Reference: P01-A response plan §S12.
