...
modified: 2026-07-16 (Manager 14): §"Negative-control wording" superseded — the label/cohort shuffle nulls were found INVARIANT to the set-valued statistic (T1.38 Phase-2 null audit, Manager-confirmed against permutation_nulls.py); their p-values were never evidence in either direction. Negative-control function moves to double-null calibration per the User's 2026-07-16 ruling (T1.6 machinery, extended to USoc + ladder rungs; successor Task T1.41 after T1.38 Phase-2 caches land). Affects §4.3 prose (held), Table 1 structure (label/cohort rows INVALID-BY-CONSTRUCTION), and P01-B §4.2 Table 2 legacy label/cohort rows. Previous record retained below. Modified by the Manager.
2026-07-11 (Manager 13): §"FDR family redefinition" corrected — BY-for-cohort → BH-for-all-three, plus the per-subgroup Markov-1 irreducibility test model and the 2/3/7 (BHPS cohort 6) subgroup counts, per the User-approved 2026-06-27 and 2026-06-28 pre-registration amendments. The prior text (BY cohort, 2/6/42 pairwise counts) was stale; T2.7 (§6.1 prose) surfaced the file-vs-Spec conflict and correctly drafted to the committed files. Affects any parallel P01-B stratified reporting (T2.13/T2.16) — do not re-introduce BY-cohort. Previous record retained below. Modified by the Manager.
2026-06-07 (Manager 9): §"Sparse U-states" updated to record the verified T1.25 outcome — both 6-state collapses fall below the ARI ≥ 0.9 robustness threshold (U_to_I 0.7022, U_to_E 0.6280); prose direction locked to sensitivity disclosure (no robustness claim), superseding the prior supportive presumption. Mechanically determined by the 2026-06-06 pre-registration. Previous record retained below. — "Locked Python environment" and "Stratified Markov-1" sections updated after T0.1 (sklearn 1.8.0 confirmed; giotto-tda removed; no GMM checkpoint in repo). jbstat section updated after T0.9 (verified, locked recoding). Income section updated after T0.10 (variable name corrected to fihhmngrs_dv; BHPS cohort wave b; calibration results added; tercile-boundary decision LOCKED — within-era boundaries (User-confirmed)). FOO clustering section updated after T0.11 (99.94% coverage confirmed). 2026-05-25 — "Final regression specification" and "Endogeneity / mediation framework" sections replaced; new top-level "Topological FOO Signature" section added after T1.21 documented structural non-estimability of cross-classified GLMM on R2/R6 subsample (92% FOO singleton dominance). Tier 3 cross-classified GLMM abandoned; replaced by Tier 2 + svyglm sensitivity (T1.34) as §4.5 headline; FOO effects addressed via topology-side per-individual local persistence + sibling-pair geometric coherence test (T1.33) as paper centrepiece; T1.21 diagnosis transparency supplement (T1.35). Formal causal mediation (Imai-Keele-Tingley) deferred to follow-up paper. Pre-registration discipline extended to T1.33, T1.34 FOO sensitivity sub-test, T1.35. Modified by the Manager.
---
...
**Vault access.** Obsidian research vault at `C:\Users\steph\Documents\TDA-Research`. **Reads** go through the `vault-engine` MCP server (`vault_get`, `vault_query`, `vault_skeleton`, `vault_status`, `vault_graph`, `cross_vault`) — these provide wikilink-graph context that raw filesystem reads do not. **Writes** (appending log entries, editing `CONVENTIONS.md`, updating `_project.md`, adding permanent notes) use `Write` / `Edit` against absolute paths under the vault root; there is no MCP `vault_write` tool. `vault_observe` attaches a short observation to a page for cross-session retrieval — it does NOT append to the page and is not a substitute for `Write`/`Edit`. Authoritative pages: `CONVENTIONS.md`, `04-Methods/Computational-Log.md`, `04-Methods/Pipeline-Overview.md`, `02-Notes/Permanent/`, `03-Papers/PXX/_project.md`.

**Existing `CLAUDE.md` content** to preserve and reference rather than duplicate: project purpose and 10-paper programme overview; Obsidian vault integration workflow; key TDA concepts and library list; code conventions (Python 3.13, 120-char lines per the root `.ruff.toml`, Ruff E/F/I/W, type hints with `numpy.typing.NDArray`, Google docstrings, research-context comment header on new scripts, random seed logging); commit prefixes ([RESULT]/[DECISION]/[NEGATIVE]/[PIPELINE]/[DATA]/[EXPLORE] with vault-action mapping); after-session vault sync; common workflows (test, lint, run pipeline, paper-start sequence); APM workflow placeholder; methodological mandates (W₂ + landscape L²; Markov-order-k; never raw-trajectory PH; never assume BHPS/USoc share variable coding); `Read`/`Grep`/`Glob` code-exploration policy; `vault-engine` MCP usage. The APM_RULES block layered into `CLAUDE.md` during the Rules step adds APM-specific execution patterns without duplicating these.
...
**Stratified Markov-1 ladder rung.** Inserted between Markov-1 and Markov-2 in the ladder as Level 4b. Root cause of regime-label collapse confirmed (T0.5): GMM pkl was fit under sklearn 1.3.2 and corrupted `predict()` under 1.8.0. Fix already in codebase: `load_regime_labels` in `run_wasserstein_battery.py` reads `gmm_labels` from `results/trajectory_tda_integration/05_analysis.json`, bypassing pkl deserialization entirely. Authoritative labels: 27,280 rows, 7 unique regimes (R0=3787, R1=7358, R2=5415, R3=3333, R4=3510, R5=1813, R6=2064), matching v1 Table 2. Input validation guards added to `_stratified_markov_shuffle` (scalar-vs-array and row-alignment checks). No pkl checkpoint required — `05_analysis.json` is the authoritative source. Run at L=5000, n_perms=100, all seven regimes represented, both H₀ and H₁, on USoc and BHPS. Outcome-contingent prose direction is **pre-registered** before the run. Reference: P01-A response plan §2 (H2); P01-B §4 (H2).
...
**ε\* knee detection.** Algorithm formalised (pseudocode in P01-B §3.4.2) and extracted as `detect_eps_star_knee()` in `run_zigzag_sensitivity.py` (T0.6). Spanning Betti comparison reported in three forms: single-ε ratio (current), AUC ratio over the full filtration, and W₂(D₀(X_t^new), D₀(X_t^*)) at matched sample sizes. ε* robustness across {0.54, 0.65, 0.70, 0.80}. **Empirical finding (T0.6):** `knee_analysis.json` gives median ε\*=0.54 (32 years, 1991–2022); 4 degenerate years (2003, 2005, 2011, 2019). The value 0.70 cited in the original H1 run is a pre-sensitivity-analysis hard-coded constant. **LOCKED (User-confirmed):** point estimate is ε\*=0.54 (data-driven median from sensitivity analysis); only data-driven values are valid. P01-A §4.3.2 and P01-B §3.4.2 cite ε\*=0.54. Robustness set {0.54, 0.65, 0.70, 0.80} unchanged. Reference: P01-B response plan §9 (M3).
...
**Negative-control wording — SUPERSEDED 2026-07-16 (T1.38 Phase-2 null audit + User ruling).** The original design ("§4.3 negative-control claim cites W₂ label-shuffle p ≈ 0.45… BHPS asymmetry label-shuffle p ≈ 0.036 reported as empirical finding") is **void**: `_label_shuffle` and `_cohort_shuffle` permute rows of the already-embedded point cloud, and Rips PH is set-valued, so both nulls are **invariant to the statistic** — their p-values are landmark-selection noise, uniform by construction, at every landmark count and era. Neither the "p ≈ 0.45 control passes" reading nor the "p ≈ 0.036 asymmetry" reading was ever evidence. **Replacement (User ruling 2026-07-16): the negative-control function moves to double-null calibration** per the T1.6 machinery (fresh null draw vs null bank; calibration ⇔ uniform p-values), extended to USoc and across the ladder rungs (successor Task T1.41, after the T1.38 Phase-2 caches land); §4.3 carries the calibration panel plus a one-sentence disclosure that group-label shuffles are inapplicable to an unlabeled statistic. Table 1's label/cohort rows are INVALID-BY-CONSTRUCTION, do-not-cite. Provenance: Computational-Log 2026-07-16 `[NEGATIVE]` + `[DECISION]`; permanent note `a-null-must-perturb-the-object-the-statistic-consumes`. Original reference retained for history: P01-A response plan §10.1.
...
**FDR family redefinition.** §6.1 stratified W₂ tests form three FDR families corrected separately — **all three by Benjamini-Hochberg (BH)**. The test model is **per-subgroup Markov-1 irreducibility** (each subgroup's observed PD vs its *own* frozen Markov-1 null; mean-vs-mean T_ratio per B2.6, BCa CI), one test per subgroup: gender 2, NS-SEC 3, cohort 7 (BHPS cohort 6). This **supersedes** the original §B8 design, which counted pairwise between-subgroup tests (2/6/42) and applied BY to the cohort family — under the per-subgroup construction the cohort tests run on *disjoint* decade subgroups and are mutually independent, so standard BH applies (gender and NS-SEC were already BH). §6.1 v2 prose frames the result as per-subgroup Markov-1 irreducibility, **not** between-subgroup difference; the legacy pairwise `06_stratified.json` numbers are preserved-not-citable. Locked by the User-approved **2026-06-27 amendment** (`results/panel_methodology/fdr/pre_registrations_amendment_2026-06-27.json`); BHPS cohort n_tests 7→6 by the **2026-06-28 amendment**. Reference: P01-A response plan §B8 (superseded on family method + counts + test model).
...
**Final regression specification: Tier 1 + Tier 2 build-up; Tier 3 cross-classified deferred.** Tier 1 (clustered SE on `hidp` + Firth) and Tier 2 (household-RE GLMM + Firth) reported in sequence in the main regression table. The originally-planned Tier 3 cross-classified GLMM with `(1|hh_group) + (1|foo_cluster)` is structurally non-estimable on the disadvantaged-starter (R2/R6) subsample due to family-of-origin singleton dominance (6,363 of 6,894 clusters singleton, ~92% — diagnosis in T1.21 log at `.apm/memory/stage-01/task-01-21.log.md`), and is abandoned as the primary regression spec. The Tier 2 household-RE captures the within-disadvantaged-starter clustering structure. A design-based estimator (`svyglm` + cluster-robust SEs at the foo_cluster level) is reported as a sensitivity column to address IPW pseudo-likelihood concerns in `glmmTMB(family=binomial, weights=w)` (non-integer trial-count warnings). Sibling-consistent MICE for parental NS-SEC is retained as a fixed-effect covariate in all tiers. Family-of-origin effects are addressed via the topology-side test in §"Topological FOO Signature" rather than via a within-regime random effect. Reference: P01-A response plan §S5, §S7, §S9.
...
**Endogeneity / mediation framework.** §S8 frames the mediation structure descriptively: parental NS-SEC correlates with first-window regime placement (cross-tabulated in supplement Table SX); first-window regime predicts escape (Table 3). The §4.5 regression treats parental NS-SEC as a fixed-effect covariate; the operating channel (additive vs. through-regime mediation) is documented as descriptive observation, not as a formal causal claim. Formal counterfactual mediation decomposition (e.g., Imai-Keele-Tingley) is out of scope for this paper and deferred to a follow-up methodological treatment. Reference: P01-A response plan §S8.
...
**Sparse U-states.** Effective dimensionality of the bigram matrix reported. 6-state (E/I × L/M/H) sensitivity rerun computed (T1.25). **Verified (T1.25, commit `2f0d8e2`, PR #39; outcome-lock [DECISION] 2026-06-07):** both pre-registered collapses fall BELOW the ARI ≥ 0.9 robustness threshold against the locked 9-state GMM labels — `U_to_I` ARI = 0.7022, `U_to_E` ARI = 0.6280 (N=27,280). The original presumption that the ARI would evidence U-states not driving results does **not** hold. **Prose direction (P01-A §S13): disclose variant-specific sensitivity; do NOT claim sparse-U robustness;** U states retained on substantive/conceptual grounds only. Reference: P01-A response plan §S13.

## Topological FOO Signature

The originally-planned Tier 3 cross-classified GLMM with `(1|foo_cluster)` is structurally non-estimable on the disadvantaged-starter subsample (~92% FOO singleton dominance; T1.21 diagnosis at `.apm/memory/stage-01/task-01-21.log.md`). Family-of-origin effects are addressed via a topology-side test that extends the paper's TDA methodology, with regression-side sensitivity and transparency supplements operating as defensive infrastructure rather than the substantive contribution.

**T1.33 — Topological FOO signature (TDA-side; centrepiece).** Per-individual local persistence features computed on the 90-D bigram embedding via k-nearest-neighbour sub-cloud PH (k pre-registered with sensitivity sweep over a candidate grid). Per-person scalars: max H₀ persistence (local density gap), total H₁ persistence (local loop content), persistence-landscape integral at k=1 (local geometric "energy"). Sibling-pair geometric coherence test on the full-sample foo_clusters: constrained-shuffle permutation null (preserves cluster-size structure, B = 5000) on multivariate sibling-pair feature-vector distance vs. random-pair feature-vector distance. Per-feature ICC reported with bootstrap CI. Output becomes a §4.5.x sub-section of P01-A — "Topological structure of family-of-origin clustering" — the substantive FOO finding of the paper, answered via topology.

Pre-registered decision rule: permutation test p < 0.05 AND any per-feature ICC CI strictly above zero → SUPPORT FOO clustering in topology, §4.5.x reports geometric FOO signature; otherwise NULL, §4.5.x reports tested-and-no-signal as an informative geometric null.

**T1.34 — Regression refit with design-based sensitivity.** Tier 2 + normalised IPW + MICE-pooled NSSEC proxy as the §4.5 headline regression (replacing the abandoned Tier 3 cross-classified spec). `svyglm` + cluster-robust SEs at the foo_cluster level reported as a sensitivity column in the same regression table to address the IPW pseudo-likelihood concern in `glmmTMB(family=binomial, weights=w)` (non-integer trial-count warnings). Single full-sample FOO sensitivity paragraph at end of §S8: full-sample `escape ~ baseline_covariates + (1 | foo_cluster)` Tier-2-style fit, reporting σ²_foo with 95% bootstrap CI as a single sentence documenting whether between-FOO variance is detectable on the broader sample.

**T1.35 — T1.21 transparency supplement.** Power analysis on n=735 multi-FOO subsample (minimum detectable ICC at 80% power, simulation-based, B=1000 iterations per ICC grid point in {0.01, 0.025, 0.05, 0.10, 0.20}); singleton decomposition of the 6,363 disadvantaged-starter singletons into true singletons (no sibling in xwavedat) vs. filtered singletons (sibling excluded by IPW + first-window R2/R6 + 10-of-14); sibling-pair concordance OR + Cohen's κ on the 353 multi-member clusters as a transparent design-based complement to the RE model. Output is a two-page supplement section explaining honestly why the cross-classified spec was infeasible and what was substituted.

**Pre-registration discipline.** Each of T1.33, T1.34's FOO sensitivity sub-test, T1.35 has a vault `[DECISION]` entry filed before run with parameters, decision rule, and prose-direction rule per outcome. The post-run `[RESULT]` entry references the pre-registration. T1.21 receives a `[NEGATIVE]` vault entry documenting the cross-classified non-estimability finding independent of T1.33–T1.35 outcomes.
