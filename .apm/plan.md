...
* **Guidance:** Use `Grep` with a regex such as `"np\\.random|numpy\\.random|default_rng"` and `type: "py"` to locate RNG calls; the project policy is to use the built-in `Read`/`Grep`/`Glob` tools for code exploration. The fix pattern is to thread a `seed` parameter through function signatures and use `np.random.default_rng(seed)` exclusively rather than module-level state. See Spec §"Deterministic seed propagation".
* **Dependencies:** Task 0.1.

1. Run `Grep` with pattern `"np\\.random|numpy\\.random|default_rng"` and `path: "trajectory_tda"` (then again for `shared/`) to locate all RNG sites; use `output_mode: "content"` with `-n` to capture file:line context.
...
1. Use `Read` on `permutation_nulls.py` (lines 168–234 region) after using `Grep` for `"def _markov"` to confirm the function boundaries.
...
* **Guidance:** See P01-A response plan §2 (H2). Likely root cause is sklearn version mismatch (resolved in Task 0.1) but verify; alternative causes are wrong checkpoint field provenance or embedding/GMM misalignment. Use `Grep` with `pattern: "_stratified_markov_shuffle"` then `Read` `permutation_nulls.py` (lines ~246–362) to inspect the function body.
* **Dependencies:** Task 0.1, Task 0.2.
...
1. Use `Grep` to locate the existing knee-detection implementation (search for `"knee"` and `"epsilon"` in `spanning_pipeline.py` / `run_zigzag_sensitivity.py`); `Read` the matched files.
...
1. `Read` `vectorisation.py` around line 232 (use `Grep` for `"wasserstein_distance"` first to locate the call site) and confirm the implementation.
...
1. `Read` `run_wasserstein_battery.py`; use `Grep` for `"def "` within the file (or with `path:` set to it) to get a function inventory before drilling in.
...
1. Audit current §5 vocabulary using `Grep` (for `"topolog"` in the §5 prose files) or the `notation-check` skill.
