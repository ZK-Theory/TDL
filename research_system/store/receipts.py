...
def _validated_receipt(record: object) -> Receipt:
    ...
    if (
        ...
        or not (outcome.get("event_batch_id") is None or isinstance(outcome.get("event_batch_id"), str))
        or not (outcome.get("reason_code") is None or isinstance(outcome.get("reason_code"), str))
    ):
        raise ConflictError("invalid idempotency index receipt")
    if status == "rejected" and (
        ...
        or not all(isinstance(item, str) and item for item in outcome["unmet_preconditions"])
        or len(outcome["unmet_preconditions"]) != len(set(outcome["unmet_preconditions"]))
    ):
        raise ConflictError("invalid idempotency index receipt")
    ...
    if status == "conflict" and (
        outcome.get("event_batch_id") is not None or outcome.get("reason_code") != "stream_version_conflict"
    ):
        raise ConflictError("invalid idempotency index receipt")
    if status == "duplicate":
        raise ConflictError("invalid idempotency index receipt")
...
def validate_scoped_receipt_index(
    record: object,
    ...
    *,
    ...
    publication_index = isinstance(record, dict) and set(record) == _PUBLICATION_INDEX_FIELDS
    ...
    if (
        ...
        or record.get("schema_version") != ("1.2.0" if publication_index else "1.1.0")
    ...
    if project_id is not None or target_stream_id is not None:
        if (
            ...
            raise IdempotencyConflictError("idempotency index target mismatch")
    if record.get("scope") != list(scope):
        raise ConflictError("idempotency index scope mismatch")
    if record.get("payload_hash") != payload_hash or record.get("authority_grant_sha256") != authority_grant_sha256:
        raise IdempotencyConflictError("idempotency key conflicts with stored outcome")
    if record["expected_stream_version"] != expected_stream_version:
        raise IdempotencyConflictError("idempotency key conflicts with expected stream version")
...
class ReceiptStore:
    ...
    def load_scoped(
        ...
        *,
        ...
        return validate_scoped_receipt_index(
            record,
            scope,
            payload_hash,
            authority_grant_sha256,
    ...
    def write_scoped(
        ...
        *,
        ...
        record = {
            ...
            "schema_version": ("1.2.0" if project_id is not None else "1.1.0"),
            "scope": list(scope),
            "payload_hash": receipt.payload_hash,
