...
| [11-portfolio-and-discovery-lifecycle.md](11-portfolio-and-discovery-lifecycle.md) | `accepted_exact_revision_external_record` | D-G6-4 limb 1 accepts exact content commit `892d1d16`, blob `f90729d0`, and corrected raw SHA-256 `65a7bc6a...40f70` through the external PR #202 owner record; the embedded `review_pending` text remains an immutable snapshot. The accepted specification has 81 owner rows and permits inert catalogue materialization only. Limb 2 remains open; no runtime, admission, projection, ingestion, transition, migration, cutover, or claim authority is granted. |
