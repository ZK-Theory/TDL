...
| `statement_hash` | Hash of the **elaborated type** of each promoted theorem (`#check`/`#print`-style output produced in a controlled environment importing only the pinned mathlib + the `statement_source` definitions), diffed against `statement_source` at review. Not a source-text hash — notation capture can make source text match while the elaborated type differs (M1). **Implementation obligation (deferred to the acceptor harness / W6, per §10):** the harness must fix a *canonical exported representation* — the exact Lean command/API, pretty-printer options, declaration ordering, namespace qualification, universe normalization, and byte serialization — and ship a reproducibility fixture that regenerates byte-identical hash input/output across repeated runs on the pinned toolchain. This design doc has no implementation authority (§Implementation authority) and does not pin those choices; it fixes only that the hash is over the elaborated type, not source text |
...
A `lean_proof` is admissible as Key A evidence only when all of the following hold. **Items 1–7 are machine-recomputed by the acceptor's re-execution above; items 8–9 are reviewer-gated Key B obligations and item 10 is structurally enforced — none of 8–10 is established by re-running the kernel:**
...
6. **Non-vacuity witness:** for every promoted theorem **with hypotheses or quantification over a possibly-empty domain**, a kernel-checked `example` exhibits an inhabitant/instance satisfying them — on the real referent data where feasible. A theorem whose hypotheses are unsatisfiable, or whose universal is vacuous over an empty domain, proves nothing about the claim (W5 §13.3 in formal clothing). Where the real-referent witness is judged infeasible, the `referent_note` records **who made that determination and why**, and states **what the substitute witness does and does not establish** — "where feasible" is not an unrecorded escape hatch (M5).
