...
These entries record directions Stephen stated in the 2026-07-16 planning session that produced `implementation/06-wp6-gate6-readiness-and-integration-plan.md`. They are registered as `proposed`: the direction is on record, but the exact wording below awaits Stephen's confirmation, and none of them alters Gate 5, which remains open and unaffected. Each is written against the evidence survey in that session (Gate A blockers A1–A8 of the vault-side TDA-scale programme v1.0.0 cross-checked against the accepted W1–W8 designs, the Gate 5 plan, and the implemented `research_system` foundation).

### P-031 — Gate 6 pilot definition amendment

**Date:** 2026-07-16<br>
**Status:** Proposed (owner direction recorded; wording confirmation pending)<br>
