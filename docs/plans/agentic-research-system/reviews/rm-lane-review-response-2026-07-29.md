...
| M-9 | **fixed** | RM-01 §Task A/B | Dispatch-head collection manifest recorded before any production mutation; the 156 handoff-28 node IDs preserved as a named historical cohort; both cohort and full current universe compared. The R1-3a signature guard was already repaired by `a681180`, before this revision, so RM-01 reverifies rather than reorders that edit. "Green" remains withdrawn until G-RM-7 resolves R1-3b. |
...
|---|---|---|
| G-RM-7 | Owner record chooses: **add** `receipt-v2` to the closed literal and provide verification evidence for the closed literal plus submit-signature guards; or **deliberate omission**, recording the reason, follow-up owner/action, and continuing non-green status until the closed-set failure is resolved. RM-01 stays non-green while either known failure is unresolved; the signature guard is fixed by `a681180` but must be reverified at dispatch head | RM-01 close-out; any "suite green" claim |
...
| G-RM-11 | Keep execution deferred, or independently accept an implemented exact-subject isolation substrate: OS enforcement, W8 grant/lease/process/stop records, attributed exact-script approval, and passing escape-control negative evidence are all required. Funding or implementation alone is not readiness | RM-05 (unwritten) |
