...
| Field | Value |
|---|---|
| Status | Planning proposal informed by accepted exact-subject evidence; **not an owner decision or implementation authorization** |
| Date | 2026-08-03 |
| Planning base / current `main` | `4f8b9b857bab1a7553af5e6ea3ef170608e7e18e` |
...
| Workflow | Standalone; portfolio certification followed by separately authorized delivery slices |

This document is a retrospective and an execution proposal. It does not change or
authorize runtime code, tests, schemas, catalogues, Jira, pull requests, reviews,
decision registers, providers, merges, Gate 6, dispatch, or research execution. The
...
The owner must accept or revise this proposal before any later-family dispatch.
...
This is the plan author's recommendation, not Stephen's `proceed` decision. No tracked
record says that KAN-65 as a whole is accepted or that P2 is authorized. The tracked
KAN-65 handoff only says that KAN-65 was `In Progress` and owned the remaining WP6.1
catalogue
...
## 7. Revised dependency-ordered remaining slices

The current accepted catalogue contains 104 unique rows. Current runtime bindings
cover 19: the original six Scope/Task rows and the 13 Message rows. The exact remaining
partition is therefore 85 rows:
...
### 7.1 Common slice rules

- Start only from an owner-named exact current-main SHA, reverified at dispatch.
- One fresh completion goal and one reviewable PR subject per slice. One serial owner
  owns all central seams; leaf workers never edit them concurrently.
...
- Use one fresh Sol Ultra exact-subject review with no author history. The family goal
  ends at the PR-ready handoff before Stephen's CodeRabbit wait.
...
### 7.2 P2 — Readiness: remaining Scope + Task + Blocker
...
### 7.3 P3 — Lease and resource ownership
...
### 7.4 P4 — Dispatch and atomic ClaimDispatch
...
### 7.5 P5 — Attempt, checkpoint, and operator control excluding recovery-gated rows
...
### 7.6 P6 — Artefact authority and consumer firewall
...
### 7.7 P7 — Review plus Decision/rule/correction
...
| KAN-65 is broader than Message | Message closes 13 rows only. KAN-65's remaining catalogue is not accepted, Done, or dispatch-authorized by PR #209. | Stephen accepts/revises this plan, then authorizes P2 only. |
...
**Stephen accepts or revises this execution plan. If he records `proceed`, dispatch P2
only from the exact then-current `main`, reverified at dispatch for ancestry, clean
state, protected identities, current interfaces, and all owner gates.**

Until that decision exists, P2-P8 remain planning rows, not authorized work.
