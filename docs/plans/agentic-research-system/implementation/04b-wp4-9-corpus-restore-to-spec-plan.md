...
| R8 | 04-plan §586 (claude/codex adapter and rendering variants) | **executing** the fake-claude/fake-codex variant rows (variant-aware harness runs) | **Gate 5 follow-on** | Matrix registers them as `execution_stage: gate5`; harness variant-aware execution is a named Gate 5 dependency (mirrors 04a O11/O12/O14/O16). Registered so it is not silently dropped. |
