...
| O12 | 06c §7 step 11 / W2 | publish the release decision as a canonical event | Delivered by WP5.3 | `PublishReleaseGateDecision` now emits exactly one ledger-allocated `ReleaseGateDecisionPublished` event through `CommandService`; durable typed manifest/control evidence resolves after restart, replay and `eval release` reject the sentinel and compare the exact published event plus full canonical evidence/authority, exact retries return the original receipt, changed payloads conflict, and false/blocked are preserved. PR #92 review remediation passes 92 focused and 568 full-suite tests. Independent Manager review/merge remains required; this closes no O15 or Gate 5 obligation. |
