...
| O12 | 06c §7 step 11 / W2 | publish the release decision as a canonical event | WP5.3 implementation complete; Manager acceptance pending | `PublishReleaseGateDecision` emits exactly one ledger-allocated `ReleaseGateDecisionPublished` event through a protected `CommandService` capability. Registered immutable producer/control snapshots resolve and re-derive after restart without checkout discovery; historical retries survive later revocation while new keys revalidate current authority; replay binds grant state at publication; and durable exclusive object/receipt publication survives interruption and concurrency. Sentinel/tamper controls fail closed, exact retries return the original receipt, changed payloads conflict, and false/blocked are preserved. PR #92 second remediation passes 412 unit plus 172 integration tests (584 total), Ruff, semantic materializers, and exact `40/15/0/302/calibrated/false/blocked`. Independent Manager acceptance/merge remains required; O15 stays open and Gate 5 stays unauthorized and unaccepted. |
