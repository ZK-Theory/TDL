...
| O12 | 06c §7 step 11 / W2 | publish the release decision as a canonical event | Delivered by WP5.3 | `PublishReleaseGateDecision` now emits exactly one ledger-allocated `ReleaseGateDecisionPublished` event through `CommandService`; replay and `eval release` re-derive and compare the full canonical decision/evidence, exact retries return the original receipt, changed payloads conflict, and false/blocked are preserved. The sentinel remains only the required unpublished input precondition. Independent Manager review/merge remains required; this closes no O15 or Gate 5 obligation. |
