...
| O12 | 06c §7 step 11 / W2 | publish the release decision as a canonical event | WP5.3 implementation evidence complete; Manager acceptance pending | `PublishReleaseGateDecision` emits exactly one ledger-allocated `ReleaseGateDecisionPublished` event through a ledger-specific, single-use `CommandService` issuance boundary. Release schemas are mandatory before persistence; strict frozen W6/W7/W8 evidence preserves exact grader identities and canonical policy preimages; actual producer/public seams fail closed; conflicting immutable-object writers serialize per revision; and authority resolution uses one verified projection. PR #92 third remediation passes 439 unit plus 172 integration tests (611 total), the 131-test WP5.3 matrix, scoped Ruff, four semantic materializers, and exact `40/15/0/302/calibrated/false/blocked`. Independent Manager acceptance/merge remains required; O15 stays open and Gate 5 stays unauthorized and unaccepted. |
