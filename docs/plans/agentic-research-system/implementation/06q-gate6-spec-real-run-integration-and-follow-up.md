...
as two serial candidates under the single KAN-105 job: `STORE-1A` owns the
physical publication, governed-code/configuration, diagnostic-precedence, and
preservation prerequisites; `STORE-1B` owns the historical binding lineage,
transaction, shared verified context, public commands, and consumer migration.
Both remain one incomplete STORE capability until the assembled public path
passes. This split creates neither a competing Gate 6 plan nor another Jira
capability job.
