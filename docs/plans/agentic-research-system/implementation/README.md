...
8. [WP6.1 schema identity, producer completeness, and historical-event protocol](06h-wp6-1-schema-identity-and-artefact-command-seam-plan.md) — **candidate under KAN-95; owner-blocked on G-RM-8 and exact-subject acceptance**. The [2026-08-09 current producer and evidence record](06h-current-producer-and-evidence-record-2026-08-09.md) binds the current 86-row runtime catalogue and complete append-site census without fabricating the non-reconstructible pre-change baseline. G-RM-9/current-producer controls are candidate work; no historical protocol is activated until Stephen makes the separate exact-evidence decision.
