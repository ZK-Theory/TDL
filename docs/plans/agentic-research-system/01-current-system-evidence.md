...
|---|---|---|---|
| Living Discovery backlog; legacy source referenced by P-004/P-032 and authoritative only within the `legacy_owned` lane; dated evidence, not authority for successor-owned W11 records or actions | `C:\Users\steph\TDL\vault\00-Meta\Discovery\_backlog.md` | 26,392 bytes; `37eec1ba6bb7929d95d5349ada2f75d93636c8356aad5dffc6a59981fc0269e7` | Mutable and legacy-written. Contains active, superseded and decision-pending prose plus legacy PROMOTE/PARK/KILL labels, which govern only items still marked `legacy_owned`; they are not W11 commands, Decisions or successor lifecycle authority. The hash is a dated observation only; no row is imported, adopted, transitioned or frozen by W11. A later transition/cutover must make a fresh handle-bound observation and use the accepted inventory/mapping contracts. |
