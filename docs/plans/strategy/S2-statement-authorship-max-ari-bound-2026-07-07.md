...
> **Review integration note (2026-07-20).** Adversarial review `adversarial-S2-max-ari-statement-review-2026-07-20.md` returned `accept_with_required_changes` and independently re-derived every constant (all match bit-for-bit). This revision applies the three Majors and the minors: **F1** adds the ARI/normalised glue statements (T5/T6) that §1's certified scope actually requires; **F2** adds the bidirectional anchoring obligation **G** (an inequality proof alone can never catch an *inflated* recorded constant); **F3** replaces the broken transpose fallback with the pre-authored **T4-coarse**. Owner decisions resolved 2026-07-21 (see header): **F6** — the stricter prereg §8.4 hammer-proof rule governs; **F7** — claim graded R2 (certification) / R3 (paper-claim promotion) and countersigned by Stephen. The sole remaining dispatch gate is the R2 referent-adequacy review by a fresh independent session (05a §3 item 9). Editing this file moves its git blob hash; `statement_source` must be re-pinned to the revised blob before the diff base is used (F-review consistency matrix).
...
**Certified scope of the normalised claim (referent honesty).** T5 certifies the ARI *upper bound* as an exact rational at the concrete B9 margins; T6 is the *generic* monotone-division lemma (`obs/ub ≤ obs/maxAri`), not an instantiation at a concrete normalised constant. The published normalised numbers (`normalised_observed_ari`, `normalised_ari_bracket`) multiply the certified bracket through the **observed ARI 0.2611807, which is not itself kernel-certified** (a float with separate T1.23c provenance — §4); it cannot be a Lean constant. So the kernel-certified core is the **pair-count bracket (T4 + W), the ARI upper bound (T5), and the transfer principle (T6)** — the normalised bracket is those combined with the uncertified observed value, and **no promoted statement asserts a certified concrete normalised constant.** This is the narrowing over the "or narrow the referent" option in the F1 finding; concrete instantiation through the observed value is not possible in-kernel.
...
4. Constants **derived in-proof AND pinned by an equality obligation** (F2); never introduced by axiom or unchecked `have`. A strict-inequality-only derivation does not satisfy this item. Attribution: **G** pins the greedy value 121,751,376, the derived pair bound 60,862,048, the margin total 27,280, and rp/cp/tp; the achievable lower bound **59,684,973 is pinned by the witness W** (`example : … = 59684973 := by decide`), not by G. Both G and W are required, each for its own constants.
