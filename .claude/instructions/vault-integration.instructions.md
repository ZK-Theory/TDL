...
2. **In Claude Code / Copilot:** Produce the vault entry text and submit it via `vault_observe` or the `vault-engine` API (never use direct filesystem writes)
3. **Manually:** Use the vault-engine tools if MCP access from your environment is unavailable
