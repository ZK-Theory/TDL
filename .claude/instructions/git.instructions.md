...
- Merge admission needs Codex to reach a terminal state on the exact head: a review naming the head, or a +1 reaction left after the head's first check started. If the gate reports the head untriggered, or reports a stale +1 or a usage limit, comment `@codex review` (after the quota resets); record an owner waiver rather than bypass the gate.
