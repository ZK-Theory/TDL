...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-22 (commit e4f2326). Confidence: 100%.
...
### Architecture
repo is a Topological Data Analysis (TDA) research engine that ingests raw financial, poverty, and trajectory datasets, processes them through multi-dimensional persistence and Morse-Smale filtration pipelines, and outputs validated topological embeddings and trend analysis reports. The system leverages a centralized schema registry to orchestrate complex mathematical transformations, ensuring data consistency across disparate research domains.
...
- `research_system` — The research_system module serves as the core Topological Data Analysis (TDA) layer of the repowise pipeline, acting as the primary…
...
- `poverty_tda/scripts/run_deprivation_scale_coherence.py` — 1 bug fix, last fix 4 days ago; 8 commits/90d

### Code health
Three co-equal signals: defect risk 9.41/10 avg, hotspot health 5.34/10 (stable), worst `poverty_tda/validation/spatial_comparison.py` at 1.0/10 · maintainability 9.56/10 · performance risk 278 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
