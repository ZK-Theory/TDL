...
## Codebase Intelligence for ci-lint-lane (Repowise)

Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-09-08 (commit 207da0d)
...
- `tests/research_system/unit/test_schema_registry.py` — 2 bug fixes, last fix 4 weeks ago; 26 commits/90d
- `research_system/projection/grandfather.py` — 2 bug fixes, last fix 4 weeks ago; 6 commits/90d
- `tests/research_system/smoke/test_wp6_1_06h_append_path_closure.py` — 2 bug fixes, last fix 4 weeks ago; 8 commits/90d
- `tests/research_system/unit/test_grandfather_prefix.py` — 2 bug fixes, last fix 4 weeks ago; 7 commits/90d
- `tests/research_system/integration/test_brief_round_trip.py` — 2 bug fixes, last fix 4 weeks ago; 10 commits/90d

### Code health
Three co-equal signals: defect risk 8.91/10 avg, hotspot health 3.8/10 (stable), worst `poverty_tda/topology/multidim_ph.py` at 1.0/10 · maintainability 9.19/10 · performance risk 328 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `tests/research_system/contracts/test_wp6_3_tdl_private_assurance_pack_contract.py` — change entropy — impact −3.0
- `research_system/command/lifecycle.py` — complex conditional (validate_exact_lifecycle_envelope) — impact −2.5
- `research_system/discovery/routes.py` — complex conditional (validate_discovery_route_registry) — impact −2.5
- `tests/research_system/contracts/test_w11_contract_materialization.py` — change entropy — impact −2.3
- `tests/research_system/integration/test_external_assurance_record_publication.py` — change entropy — impact −2.3

### Standing decisions (ask `get_why` before diverging)
- .agents/skills/ is the single authoring source of truth; .claude/skills/ is a byte-mirror produced…
