...
Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-29 (commit 88e5f8a). Confidence: 100%.
...
### Architecture
repo is a Topological Data Analysis (TDA) research engine: it ingests raw financial, poverty, and trajectory datasets, processes them through multi-dimensional persistence and Morse-Smale filtration pipelines, and outputs validated topological features and predictive model embeddings for systemic risk assessment. The repository is a polyglot research environment primarily driven by Python for computational logic and data science, supported by extensive JSON/YAML configurations for schema management and Markdown for documentation. The system is modularized by domain-specific research focus, utilizing a shared utility layer to maintain consistency across disparate data types.
...
- `research_system` — The research_system module serves as the core Topological Data Analysis (TDA) subsystem of repowise, acting as the primary orchestration…
...
- `tests/research_system` — The tests/research_system module serves as the verification and validation (V&V) layer for the research system's orchestration pipeline —…
...
- `trajectory_tda/scripts/run_stage1_aux_diagnostics.py` — 2 bug fixes, last fix 5 weeks ago; 8 commits/90d
- `trajectory_tda/scripts/stage1/_battery_core.py` — 2 bug fixes, last fix 4 weeks ago; 11 commits/90d
- `research_system/evals/fixture_package.py` — 1 bug fix, last fix today; 6 commits/90d
- `tools/sync_agent_skills.py` — 1 bug fix, last fix 7 days ago; 9 commits/90d

### Code health
Three co-equal signals: defect risk 9.36/10 avg, hotspot health 5.83/10 (stable), worst `poverty_tda/validation/spatial_comparison.py` at 1.0/10 · maintainability 9.55/10 · performance risk 292 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
