...
- Hook scripts: `.claude/hooks/` (notation-guard, results-no-overwrite,
  dispatch-readiness-guard, mirror-tree-guard, admin-bypass-guard,
  research-context-check, results-vault-reminder); git hooks: `.githooks/`

<!-- REPOWISE:START — Do not edit below this line. Auto-generated by Repowise. -->
## Codebase Intelligence for TDL (Repowise)

Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-13 (commit 2266cb8). Confidence: 100%.
...
### Architecture
repo is a Topological Data Analysis (TDA) research engine that ingests raw financial and socioeconomic datasets, processes them through multi-dimensional persistence and Morse-Smale filtration pipelines, and outputs structured topological embeddings and trend validation reports. The system orchestrates complex mathematical transformations—including N-gram embedding and persistent homology—to map high-dimensional trajectory data into interpretable analytical schemas.
...
- `research_system` — The research_system module serves as the core Topological Analysis Engine within repowise, acting as the primary orchestration layer that…
...
- `research_system/projection/grandfather.py` — 2 bug fixes, last fix 3 days ago; 6 commits/90d
- `tests/research_system/unit/test_schema_registry.py` — 2 bug fixes, last fix 3 days ago; 23 commits/90d
- `tests/research_system/smoke/test_wp6_1_06h_append_path_closure.py` — 2 bug fixes, last fix 3 days ago; 8 commits/90d
- `tests/research_system/unit/test_grandfather_prefix.py` — 2 bug fixes, last fix 3 days ago; 7 commits/90d
- `tests/research_system/integration/test_brief_round_trip.py` — 2 bug fixes, last fix 3 days ago; 10 commits/90d

### Code health
Three co-equal signals: defect risk 9.05/10 avg, hotspot health 5.2/10 (stable), worst `poverty_tda/validation/spatial_comparison.py` at 1.0/10 · maintainability 9.31/10 · performance risk 308 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.
...
- `research_system/command/lifecycle.py` — complex conditional (validate_exact_lifecycle_envelope) — impact −2.5
