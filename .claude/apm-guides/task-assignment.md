...
7. Direct the User to the Worker's chat per `.claude/skills/apm-communication/SKILL.md` §2.1 Direct Communication:
...
- *Input Provenance Ledger:* Included for Tasks that consume input data (per §2.2 Input-Provenance Gate). The pasted output of `manager_predispatch_check.py` over the Task's input-provenance manifest — each input's path, root, on-disk signature, and the coherence verdict — generated from a fresh on-disk check at dispatch, never written from memory. Names the manifest the Worker's commit gate re-asserts.
