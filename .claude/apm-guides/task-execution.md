...
**Fail-fast:** If any Task results in Failed status, stop the batch. Do not proceed to remaining Tasks. After completing all Tasks (or stopping on failure), write a single batch report to the Report Bus per `.claude/apm-guides/task-logging.md` §4.3 Batch Report Format. Do not defer logging to the end of the batch.
...
2. Verify `agent` in YAML frontmatter matches your assigned identity. Validate the bus directory matches `agent` per `.claude/skills/apm-communication/SKILL.md` §4.1 Bus Identity Standards. If mismatch, decline per `.claude/commands/apm-3-initiate-worker.md` §5 Operating Rules.
...
Perform the following actions:
1. Execute Detailed Instructions sequentially, applying Guidance and relevant Rules from `CLAUDE.md`, working toward the Objective.
...
4. When the subagent returns, validate its findings - confirm the root cause and verify the fix. If sound, apply and return to Task Validation. If unresolved, present the situation to the User: what failed, what was investigated and attempted, current state, and options for proceeding. Upon User guidance, integrate the new direction or apply outcome status per `.claude/apm-guides/task-logging.md` §2.2 Outcome Standards and continue to Task Completion.

### 3.6 Task Completion

Perform the following actions:
1. Present your assessment visibly in chat: whether all objectives are met and deliverables are ready, whether any important findings or compatibility issues arose, and the Task's outcome status per `.claude/apm-guides/task-logging.md` §2.2 Outcome Standards.
2. Commit work to the assigned branch per §2.5 Version Control Standards.
3. Create Task Log per `.claude/apm-guides/task-logging.md` §3.1 Task Log Procedure at `log_path`.
4. Write Task Report per `.claude/apm-guides/task-logging.md` §3.2 Task Report Delivery. Include relevant status indications:
