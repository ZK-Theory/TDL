...
This resolves the earlier 'divergent' result, in which the single-$\varepsilon$ ratio and $W_2$ pointed one way while a *full*-interval $\beta_0$-AUC ratio (0.938–0.939, i.e. below parity) pointed the other. The divergence was an artefact of integrating the Betti-0 curve over the full $[0, \varepsilon^*]$ interval, where the near-zero-scale regime dominates; the windowed AUC isolates the discriminating band and removes the contradiction. The pre-registered decision rule — locked-$\varepsilon$ windowed-AUC p < 0.05, locked-$\varepsilon$ bootstrap CI excluding parity, direction consistency across all $\varepsilon^*$, and significant $W_2$ — is satisfied, yielding the outcome: **newcomers carry more $H_0$ structure, robustly across $\varepsilon^*$ and corroborated by $W_2$**.
...
Taken with the pool-draw null (§4.3.1), the spanning-individual evidence supports the survey-frame-expansion reading of the dominant post-2008 $H_0$ change: spanning individuals retain era-1-like topology while newcomers, who carry significantly and robustly more $H_0$ structure, occupy embedding regions the BHPS frame under-sampled. The strength of this reading is currently bounded by the pending matched/age-stratified confirmation described above.
