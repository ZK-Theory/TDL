...
The restriction to $H_0/H_1$ is maintained as a protocol deviation from the planned $L=2{,}000$ check. The documented $L=1{,}000$ fallback was attempted with the automatic threshold, but its rejection was confounded by dispersion and filtration-scale effects and therefore was not a scale-comparable confirmation; a pinned-threshold $L=1{,}000$ run also exceeded the tetrahedron budget. The available small-$L$ characterization is modest in magnitude, not robust to the diameter-normalization choice, and cannot establish the intended high-landmark result. No dedicated higher-homology section is added; conclusions in this paper are restricted to $H_0/H_1$, with the unavailable $H_2$ confirmation treated as a limitation rather than as support for the pre-registered decision rule.
