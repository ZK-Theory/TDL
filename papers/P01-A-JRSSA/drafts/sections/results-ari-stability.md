...
causal claim and no claim that either method uniquely identifies the regimes. Reporting
the agreement in normalised, null-tested, interval-bearing form does not upgrade the
substantive claim.

## Table 2 — Regime stability with uncertainty
...
**Table 2 (augmented) — Seven Mobility Regimes, stability with uncertainty**
...
transition count rather than its member count — was also computed; the member-count
denominator is the headline.

## Table 3 — Escape rates with Wilson 95% CIs

Every escape rate carries a Wilson 95% interval. The retirement-age rate in particular
spans a wide relative range, reflecting the small number of escapes in that group.

**Table 3 (augmented) — Escape Rates from Disadvantaged Regimes**
