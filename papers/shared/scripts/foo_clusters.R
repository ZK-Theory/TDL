...
out_dir       <- "c:/Users/steph/TDL/data/derived"
...
xhhrel <- read.delim(fpath_xhhrel, header = TRUE, sep = "\t",
...
canon <- t(apply(all_edges, 1, sort))
all_edges_dedup <- unique(data.frame(from = canon[, 1], to = canon[, 2]))
...
next_cluster_id <- max(foo_linked$foo_cluster) + 1L
...
foo_df <- rbind(foo_linked, foo_singletons)
...
cc_xw[idx_xw] <- NA
xwave_pidp_df <- read.delim(fpath_xwave, header = TRUE, sep = "\t",
...
pct_cov  <- round(length(in_both) / length(xwave_pidp_all) * 100, 2)
...
pct_linked <- round(linked_in_both / length(xwave_pidp_all) * 100, 2)
...
if (nrow(icc_df) > 100) {
  ...
  k_avg <- mean(clust_n)
  icc_val <- (MSB - MSW) / (MSB + (k_avg - 1) * MSW)
  cat("  ICC (within FOO clusters, gross income):", round(icc_val, 4), "\n")
  cat("  Interpretation: ICC", round(icc_val, 4), "—",
...
out_json_dir <- "c:/Users/steph/TDL/results/panel_methodology/harmonisation"
...
result <- list(
  ...
  icc_verdict         = if (!is.na(icc_val))
    ifelse(icc_val > 0.05, "ACCOUNT: ICC > 0.05, FOO clustering material",
           "MARGINAL: ICC <= 0.05, FOO effect weak") else "insufficient data"
)
