...
def test_normalize_landmarks_by_median_pdist_returns_unit_median() -> None:
...
def test_h2_dispersion_normalization_scale_invariance_contract() -> None:
    ...
    for scale_factor in (0.5, 2.0):
        scaled_norm, _ = diagnostic._normalize_landmarks_by_median_pdist(base * scale_factor)
...
def test_run_condition_passes_normalization_flag_to_null_worker(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda index, **kwargs: (
            seen.setdefault("normalize", kwargs["normalize"]) and index,
