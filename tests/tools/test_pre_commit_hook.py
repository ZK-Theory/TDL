...
""",
    ...
    # One entry per main-checkout gate, in hook order: gate 0, gate 1b, gate 2. Gate 1
...
STAGED_GENERATED = "staged Repowise-generated bytes"
DANGLING_GITLINK = "gitlink with no .gitmodules entry"


def _admission_fixture(tmp_path: Path) -> tuple[Path, dict[str, str]]:
    """A repo whose gates all pass, so only staged-set admission can refuse."""
    repo, env = _repowise_fixture_repo(tmp_path, ())
    _write_fake_interpreter(repo, "")
    env.pop("TDL_ALLOW_REPOWISE_STAGE", None)
    return repo, env


@pytest.mark.integration
@pytest.mark.skipif(_git_bash() is None, reason="Git Bash is required")
def test_pre_commit_refuses_a_staged_repowise_block(tmp_path: Path) -> None:
    """Obs 2026-09-08-git-add-dash-A-swept-in-generated-metadata: a broad add sweeps in the index block."""
    repo, env = _admission_fixture(tmp_path)
    claude = repo / ".claude" / "CLAUDE.md"
    claude.write_text(CLAUDE_MD_BODY.replace("aaaaaaa", "bbbbbbb"), encoding="utf-8", newline="\n")
    _git(repo, "add", "-A")

    refused = _run_hook(repo, env)
    env["TDL_ALLOW_REPOWISE_STAGE"] = "1"
    admitted = _run_hook(repo, env)

    assert refused.returncode == 1, refused.stderr
    assert STAGED_GENERATED in refused.stderr
    assert ".claude/CLAUDE.md" in refused.stderr
    assert admitted.returncode == 0, admitted.stderr
    assert "TDL_ALLOW_REPOWISE_STAGE=1" in admitted.stderr


@pytest.mark.integration
@pytest.mark.skipif(_git_bash() is None, reason="Git Bash is required")
def test_pre_commit_refuses_a_staged_repowise_workspace_file(tmp_path: Path) -> None:
    repo, env = _admission_fixture(tmp_path)
    (repo / ".repowise-workspace.yaml").write_text(
        WORKSPACE_BODY.replace("aaaaaaaaaaaaaaaa", "bbbbbbbbbbbbbbbb"), encoding="utf-8", newline="\n"
    )
    _git(repo, "add", "-A")
    ...
    assert refused.returncode == 1, refused.stderr
    assert STAGED_GENERATED in refused.stderr
    assert ".repowise-workspace.yaml" in refused.stderr


@pytest.mark.integration
@pytest.mark.skipif(_git_bash() is None, reason="Git Bash is required")
def test_pre_commit_admits_a_hand_edit_outside_the_generated_block(tmp_path: Path) -> None:
    """Positive control: CLAUDE.md policy text above the block is ordinary authored content."""
    repo, env = _admission_fixture(tmp_path)
    claude = repo / ".claude" / "CLAUDE.md"
    claude.write_text(
        CLAUDE_MD_BODY.replace("# CLAUDE.md\n", "# CLAUDE.md\n\nA new rule.\n"), encoding="utf-8", newline="\n"
    )
    _git(repo, "add", ".claude/CLAUDE.md")
...
@pytest.mark.integration
@pytest.mark.skipif(_git_bash() is None, reason="Git Bash is required")
def test_pre_commit_refuses_a_gitlink_without_a_gitmodules_entry(tmp_path: Path) -> None:
    """Obs 2026-09-08-disabled-ci-let-a-checkout-breaking-defect-live-three-months: a bare
    gitlink broke every fresh checkout for three months. It must not reach a commit again."""
    repo, env = _admission_fixture(tmp_path)
    ...
    (repo / ".gitmodules").write_text(
        '[submodule "vendored"]\n\tpath = vendored\n\turl = https://example.invalid/vendored.git\n',
        encoding="utf-8",
        newline="\n",
    )
    ...
    assert "vendored" in refused.stderr
    assert admitted.returncode == 0, admitted.stderr
