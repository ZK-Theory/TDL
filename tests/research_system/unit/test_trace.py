...
def test_trace_requires_terminal_receipt_or_missing_evidence_for_resources():
    trace = _trace(issued_resources=({"resource_request_id": "rrq-1"},))
    with pytest.raises(ValueError, match="terminal_or_missing_evidence_required"):
        trace.validate_terminal_evidence()


@pytest.mark.parametrize("terminal_field", ["terminal_ref", "missing_evidence_ref"])
def test_trace_accepts_explicit_resource_terminal_or_missing_evidence(
    terminal_field,
):
    trace = _trace(
        issued_resources=(
            {
                "resource_request_id": "rrq-1",
                terminal_field: "resource-record-1",
            },
        )
    )
    trace.validate_terminal_evidence()
