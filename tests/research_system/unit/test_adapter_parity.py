...
def test_real_typed_fake_evidence_closes_every_control_provider_pair():
    execution = run_p0_coverage(EVALS / "p0-coverage.yaml", fixture_root=EVALS / "fixtures", schema_root=ROOT / ".research-system" / "schemas")
...
def test_plain_or_self_attested_manifest_is_rejected():
    ...
    applicability = load_policy_control_applicability(POLICIES / "gate5-policy-control-applicability.yaml", bundle=bundle)
...
def test_missing_or_diagnostic_critical_evidence_cannot_be_averaged_to_pass():
    execution = run_p0_coverage(EVALS / "p0-coverage.yaml", fixture_root=EVALS / "fixtures", schema_root=ROOT / ".research-system" / "schemas")
    bundle = load_canonical_policy_bundle(POLICIES / "canonical-policy.yaml")
    ...
    diagnostic = (*execution.parity_evidence[:-1], replace(execution.parity_evidence[-1], disposition="diagnostic_only"))
