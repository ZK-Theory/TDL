...
class _Snapshot:
    def __init__(self, failures):
        ...
        self._failures = {profile_id: tuple(reasons) for profile_id, reasons in failures.items()}
...
def test_candidate_permutation_produces_identical_selection_data():
    ...
    snapshot = _Snapshot({})
    forward = _select_route(request, candidates, snapshot)
    reverse = _select_route(request, list(reversed(candidates)), snapshot)
...
def test_live_telemetry_cannot_change_bound_snapshot():
    ...
    live["a"] = ()
    result = _select_route(_Request("rrq_" + "3" * 32), [_candidate("a")], snapshot)
