...
def _registry(tmp_path):
    return EvidenceStoreRegistry(
        ...
        verifier_actor_ids=('actor-1',),
        verifier_authority_grant_ids=('grant-1',),
...
def test_failed_inaccessible_or_reparse_location_stays_pending(tmp_path):
    ...
    def inspect(path, evidence_hash):
        ...
        if path.name == 'runtime':
            return LocationInspection(
                **(asdict(result) | {'reparse_target': str(tmp_path / 'escape')})
            )
        return result
    ...
    manifest = _verify(
        ...
        registry=EvidenceStoreRegistry(
            **(
                ...
                | {
                    'verifier_actor_ids': (ACTORS['actor-a'],),
                    'verifier_authority_grant_ids': (AUTHORITY_GRANT_ID,),
                }
            )
        ),
    )
