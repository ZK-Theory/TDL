...
def test_canonical_policy_keeps_critical_controls_provider_neutral():
    policy = yaml.safe_load(
        (ROOT / ".research-system" / "policies" / "canonical-policy.yaml").read_text(encoding="utf-8")
    )
...
def test_provider_manifests_are_argument_arrays_and_live_disabled():
    for provider in ("claude", "codex"):
        manifest = yaml.safe_load(
            (ROOT / ".research-system" / "adapters" / f"{provider}.yaml").read_text(encoding="utf-8")
        )
...
def test_checkpoint_manifest_rejects_malformed_hashes(field):
    ...
    with pytest.raises(SchemaError, match=field):
        SchemaRegistry(ROOT / ".research-system" / "schemas").validate("ars://operations/checkpoint-manifest", manifest)
