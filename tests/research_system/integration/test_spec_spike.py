...
APPROVAL_EVIDENCE = {
    ...
    "cost_ceiling": {"time_limit_seconds": 600, "worker_limit": 2, "memory_limit_mb": 1024, "network_access": False},
}
PLAN_EVIDENCE = {
    ...
    "time_resource_box": {"time_limit_seconds": 60, "worker_limit": 1, "memory_limit_mb": 512, "network_access": False},
}
...
def _promoted(bound, tmp_path, capsys, source_repo, monkeypatch) -> str:  # noqa: F811
...
def test_public_spec_02_path_starts_the_approved_spike(tmp_path, monkeypatch, capsys, source_repo):  # noqa: F811
    ...
    candidate_id = _promoted(bound, tmp_path, capsys, source_repo, monkeypatch)
    ...
    assert coordinator._operational_state()[c1.ATTEMPT_ID]["status"] == "running"

    # The listing re-derives every state from the ledger alone.
    spec_02_actions = (spec_assay.APPROVE_02, spec_assay.PREPARE_02, spec_assay.START_02)
    states = {
    ...
    assert states == dict.fromkeys(spec_02_actions, "completed")
...
def test_admission_accepts_the_spike_planning_collapses_the_route_refuses(tmp_path, monkeypatch, capsys):
    ...
    resource = coordinator._operational_state()[c1.RESOURCE_GRANT_ID]
