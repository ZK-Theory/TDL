...
_OWNER_TDA_RUNTIME_ROOT = Path.home() / "TDL"
TDA_RUNTIME_ROOT = Path(os.environ.get("TDL_REPOSITORY_ROOT", _OWNER_TDA_RUNTIME_ROOT))
...
_UNAVAILABLE_REAL_DOSSIER_ROOTS = tuple(
    name
    for name, path in (
        ("TDL_REPOSITORY_ROOT", TDA_RUNTIME_ROOT),
        ("TDA_VAULT_ROOT", VAULT),
        ("repository contract root", CONTRACT_ROOT),
    )
    if not _accessible_directory(path)
)
if _REQUIRE_REAL_DOSSIER and _UNAVAILABLE_REAL_DOSSIER_ROOTS:
    unavailable = ", ".join(_UNAVAILABLE_REAL_DOSSIER_ROOTS)
    raise pytest.UsageError(f"real TDA dossier certification requires accessible roots: {unavailable}")

pytestmark = [
    pytest.mark.integration,
    pytest.mark.skipif(
        bool(_UNAVAILABLE_REAL_DOSSIER_ROOTS),
...
def test_public_non_p0_dossier_command_rejects_without_ledger_or_receipt_mutation(tmp_path: Path) -> None:
    ...
    # Activate the deterministic governed seam before taking the mutation snapshot.
    with pytest.raises(IntegrityError, match="accepted dossier authority is not active"):
        runtime.submit(
            _command(
    ...
    command = _command(
        ...
        0,
        {
            ...
            "expected_set_id": "expected:tda-scale:1.0.3",
            "candidate_members": [],
            "candidate_manifest": {"unsafe_number": 0.5},
        },
    )
