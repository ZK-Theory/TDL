...
SCOPE_RELATIONSHIP_ID = "rel_scope_0001"
REVIEW_RELATIONSHIP_ID = "rel_review_0001"
...
def _proposed_pack(contract: dict | None = None) -> dict:
    ...
    return {
        ...
        "currency": {
            "authored_at": "2026-07-18T08:00:00Z",
            "effective_at": "2026-07-18T09:00:00Z",
...
def _validate_proposed_pack(
    ...
    *,
    ...
    if require_active_references and any(not row["pack_acceptance_eligible"] for row in observed_references.values()):
...
def _pack_subject(pack: dict) -> dict:
    candidate_bytes = _canonical_bytes(pack)
    return {
        ...
        "pack_git_blob": _git_blob_id(candidate_bytes),
        "pack_canonical_sha256": hashlib.sha256(candidate_bytes).hexdigest(),
...
def _external_records(pack: dict) -> tuple[dict, dict[str, dict], dict[str, str]]:
    requirement_record = {
    ...
    review_relationship = {
        ...
        "object_actor_id": ACT_PRODUCER,
        "grade": "I2",
        "effective_at": "2026-07-18T09:30:00Z",
    ...
    return pack, record_store, hash_manifest


def _resolved_record(record_store: dict[str, dict], hash_manifest: dict[str, str], record_id: str) -> dict:
...
def _validate_external_acceptance(
    ...
    *,
    ...
    requirement = _resolved_record(record_store, hash_manifest, requirement_id)
    ...
    scope_relationship = _resolved_record(record_store, hash_manifest, requirement["scope_relationship_record_id"])
    review = _resolved_record(record_store, hash_manifest, review_record_id)
    review_relationship = _resolved_record(record_store, hash_manifest, review["relationship_record_id"])
    owner = _resolved_record(record_store, hash_manifest, owner_decision_id)
    grant = _resolved_record(record_store, hash_manifest, owner["authority_grant_id"])
    registered_object = _resolved_record(record_store, hash_manifest, pack["assurance_pack_id"])
    subject = _pack_subject(pack)
    ...
    if scope_relationship != {
        "record_type": "producer_relationship_evidence",
        "subject_actor_id": requirement["scope_reviewer_actor_id"],
        "object_actor_id": pack["producer_actor_id"],
        "grade": requirement["minimum_independence_grade"],
        "effective_at": scope_relationship["effective_at"],
        "expires_at": scope_relationship["expires_at"],
    }:
        raise CandidatePackError("scope relationship does not bind reviewer and producer")
    if (
        review_relationship["subject_actor_id"] != review["reviewer_actor_id"]
    ...
    if grant != {
        "record_type": "active_authority_grant",
        "authority_grant_id": owner["authority_grant_id"],
        "actor_id": owner["acceptor_actor_id"],
        "authority_class": "stephen_reserved_owner_acceptance",
        "subject_assurance_pack_id": pack["assurance_pack_id"],
    ...
    if reviewed_at >= decided_at:
        raise CandidatePackError("independent review must precede owner acceptance")
    for relationship in (scope_relationship, review_relationship):
        if not _parse_datetime(relationship["effective_at"]) <= as_of < _parse_datetime(relationship["expires_at"]):
            raise CandidatePackError("relationship evidence is not current")
    if not _parse_datetime(grant["effective_at"]) <= as_of < _parse_datetime(grant["expires_at"]):
...
def test_external_acceptance_requires_independent_exact_subject_records():
    ...
    pack, record_store, hash_manifest = _external_records(_proposed_pack(contract))
    ...
    same_author_hashes = deepcopy(hash_manifest)
    same_author[REQUIREMENT_RECORD_ID]["requirement_author_actor_id"] = ACT_PRODUCER
    _rehash_record(same_author, same_author_hashes, REQUIREMENT_RECORD_ID)
    same_author_pack = deepcopy(pack)
    same_author_pack["assurance_requirement_reference"]["acceptance_record_sha256"] = same_author_hashes[
        REQUIREMENT_RECORD_ID
    ]
    same_author_subject = _pack_subject(same_author_pack)
    same_author[REVIEW_RECORD_ID]["subject"] = deepcopy(same_author_subject)
    _rehash_record(same_author, same_author_hashes, REVIEW_RECORD_ID)
    same_author[OWNER_DECISION_ID]["subject"] = deepcopy(same_author_subject)
    same_author[OWNER_DECISION_ID]["review_record_sha256"] = same_author_hashes[REVIEW_RECORD_ID]
    _rehash_record(same_author, same_author_hashes, OWNER_DECISION_ID)
    ...
    wrong_review_subject[REVIEW_RECORD_ID]["subject"]["pack_canonical_sha256"] = "f" * 64
...
def test_coordinated_candidate_and_oracle_replacement_does_not_change_external_authority():
    ...
    eligible_contract = _eligible_contract()
    pack, record_store, hash_manifest = _external_records(_proposed_pack(eligible_contract))
    tampered_pack = deepcopy(pack)
    tampered_pack["currency"]["expires_at"] = "2028-07-18T09:00:00Z"
    tampered_review = deepcopy(record_store)
    tampered_hashes = deepcopy(hash_manifest)
    tampered_subject = _pack_subject(tampered_pack)
    tampered_review[REVIEW_RECORD_ID]["subject"] = deepcopy(tampered_subject)
    _rehash_record(tampered_review, tampered_hashes, REVIEW_RECORD_ID)
    tampered_review[OWNER_DECISION_ID]["subject"] = deepcopy(tampered_subject)
    tampered_review[OWNER_DECISION_ID]["review_record_sha256"] = tampered_hashes[REVIEW_RECORD_ID]
    _rehash_record(tampered_review, tampered_hashes, OWNER_DECISION_ID)
