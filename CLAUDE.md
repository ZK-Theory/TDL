...
- **Worktree pre-removal checklist (mandatory):** Before `git worktree remove`, the Manager must verify: (1) all committed result files on the branch have been merged to `main`; (2) all gitignored intermediate files needed by downstream tasks are present at their `PROJ_ROOT` paths; (3) the manual-sweep cadence has fired (see next bullet). If any downstream-needed gitignored file is missing from `PROJ_ROOT`, regenerate it before removing the worktree.
- **Worktree removal cadence (mandatory; overrides `.claude/apm-guides/task-review.md` §2.5 Cleanup):** Post-merge `git worktree remove` and `git branch -d` are NOT performed immediately after merge. Reason: CodeRabbit's review process on the corresponding PR sometimes continues after the merge commit lands; immediate removal interrupts or bypasses CodeRabbit reviews that the project relies on for safety-net feedback (CodeRabbit can be pernickety but has caught glaring errors and failure states historically). Cleanup is **manual** — at the start of each Manager session, or on explicit User trigger (e.g., "sweep worktrees"). When sweeping, remove only worktrees whose PRs have closed AND whose CodeRabbit reviews have concluded (look for CodeRabbit's "Actionable comments posted" / "Review summary" pattern in the PR comments via `gh pr view <PR#> --json comments`). The standard pre-removal checklist (items 1 and 2 above) still applies to each sweep candidate. List swept worktrees in the Tracker working notes for the session. Manager does NOT auto-remove on a timer or merge-event basis — this is a hard block on automated cleanup.
