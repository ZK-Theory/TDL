...
**CONVENTIONS.md is repo-canonical (single source for all agents).** The always/never rules live in the committed **repo-root `CONVENTIONS.md`**; the vault's copy is a **symbolic link** to the repo file (created 2026-07-18, replacing the former hardlink). A symlink resolves by *path*, so it is rewrite-proof: when git rewrites the repo file (new inode, same path) on any commit/merge/checkout, the link still points at the current file. The old hardlink coupled by *inode* and broke on every such git write. **Load `CONVENTIONS.md` at session start; read/edit it at the repo root, never via the vault path.** Newly-locked decisions are added to the repo-root file and surface in the vault through the symlink. If the link is ever lost, recreate it from a **non-admin** shell with `cmd /c mklink "<vault>\CONVENTIONS.md" "<repo>\CONVENTIONS.md"` (Windows Developer Mode is on; `cmd mklink` honors it, but PowerShell `New-Item -ItemType SymbolicLink` and `ln -s` do **not** — they demand admin). Only ever symlink the vault side; the repo side must stay a real git-tracked file.
