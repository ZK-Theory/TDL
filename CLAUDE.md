...
- **UKDA user guides** for UKDA-6614 are available as plain-text conversions at `c:\Users\steph\TDL\data\guides\6614\` (gitignored but present on disk — use `Read` tool at absolute path). Key files: `6614_main_survey_bhps_harmonised_user_guide.md`, `6614_main_survey_user_guide_family_matrix_xhhrel.md`, `6614_Understanding_Society_and_its_income_data.md`, `6614_main_survey_user_guide_weighting_variables.md`. The PDF Viewer MCP tool is cloud-hosted and cannot access local files — always use `Read` for local documentation.
